...
docker: # Build the docker image | 构建 docker 镜像
	docker build -f Dockerfile -t  $(DOCKER_REPO)/$(SERVICE_DASH)-tenant-$(PROJECT_BUILD_SUFFIX):$(VERSION) .
	@echo "Build docker successfully"

.PHONY: publish-docker
publish-docker: # Publish docker image | 发布 docker 镜像
	docker push  $(DOCKER_REPO)/$(SERVICE_DASH)-tenant-$(PROJECT_BUILD_SUFFIX):$(VERSION)
...
gen-ent: # Generate Ent codes | 生成 Ent 的代码
	go run  entgo.io/ent/cmd/ent generate --template glob="./ent/template/*.tmpl" ./ent/schema --feature $(ENT_FEATURE)
